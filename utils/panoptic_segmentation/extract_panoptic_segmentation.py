...
from datasets.utils import get_meta_data_classes
import pandas as pd
...
if __name__ == "__main__":
    ...
    parser.add_argument('--single', default=False, action="store_true", help="only process first frame")
    args = parser.parse_args()
    frames = sorted(glob.glob("{}/*.png".format(args.image_folder)) + \
        glob.glob("{}/*.jpg".format(args.image_folder)))
    if args.single: frames  = frames[:1]
