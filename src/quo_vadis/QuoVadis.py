...
class QuoVadis():
    ...
    def run(self,
        ...
        for frame in tqdm(frames):
            ...
            if self.visibility:
                ...
                visibility = map_dict["visibility_img"]
            ...
            if social_interactions:
                ...
                existing_alive_tracks = [track for track in self.m.existing_alive_tracks(
                    frame) if (track.has_prediction and len(track.active_predictions) > 0)]
        ...
        if save_results:
            ...
            self.save_file = os.path.join(results_folder, "{}.txt".format(
                self.sequence.name))
    ...
    def plot_frame(self, frame, preds, color=None, show=False,
        ...
        if (trajectories or show_visibility):
            ...
            # print(ground_img.shape, ground_mask.shape)
    ...
    def run_eval(self):
        ...
        bash_eval_file = "./submodules//TrackEval/eval_bash_scripts/eval_model.sh"
        if not os.path.isfile(bash_eval_file):
            ...
            The evaluation file does not exists. There might be 2 possible probelms \\
                ...
                2. You are missing the 'TrackEval' submodule. To set up the module please clone repository with \\
                    `git submodule update --init submodules//TrackEval`""")
    ...
    def run_analysis(self):
        ...
        output_path = "/storage/user/dendorfp/{}/prediction_results/{}/{}/output/{}/{}".format(
            self.sequence.dataset, model, self.tracker.name, self.sequence.name, self.tracker.name)
        data_path = "/storage/user/dendorfp/{}/prediction_results/{}/{}/data/".format(
            self.sequence.dataset, model, self.tracker.name)

        baseline_output_path = "/storage/user/dendorfp/{}/tracker/{}/output/{}/{}".format(
            self.sequence.dataset, self.tracker.name, self.sequence.name, self.tracker.name)
        baseline_data_path = "/storage/user/dendorfp/{}/tracker/{}/data".format(
            self.sequence.dataset, self.tracker.name, self.tracker.name)
        sequence = self.sequence.name
        idsw_path = os.path.join(
            output_path, "idsw", "{}.txt".format(sequence))
        idtr_path = os.path.join(
            output_path, "idtr", "{}.txt".format(sequence))
        matches_path = os.path.join(
            output_path, "matches", "{}.txt".format(sequence))
        prediction_path = os.path.join(data_path, "{}.txt".format(sequence))

        baseline_idsw_path = os.path.join(
            baseline_output_path, "idsw", "{}.txt".format(sequence))
        baseline_matches_path = os.path.join(
            baseline_output_path, "matches", "{}.txt".format(sequence))
        baseline_path = os.path.join(
            baseline_data_path, "{}.txt".format(sequence))
        baseline_idtr_path = os.path.join(
            baseline_output_path, "idtr", "{}.txt".format(sequence))
        ...
        baseline = baseline_idsw.merge(baseline_matches, left_on=[
                                       "frame", "tracker_id"], right_on=["frame", "tracker_id"], how="left")
        baseline_data = baseline_data.merge(baseline_matches[["frame", "tracker_id", "gt_id"]], left_on=[
                                            "frame", "id"], right_on=["frame", "tracker_id"], how="left")

        baseline_idsw = baseline_idsw.merge(baseline_matches, left_on=[
                                            "frame", "tracker_id"], right_on=["frame", "tracker_id"], how="left")
        ...
        prediction_idsw = prediction_idsw.merge(prediction_matches, left_on=[
                                                "frame", "tracker_id"], right_on=["frame", "tracker_id"], how="left")
        prediction_idsw["gap"] = None

        prediction = prediction.merge(prediction_matches[["frame", "tracker_id", "gt_id"]], left_on=[
                                      "frame", "id"], right_on=["frame", "tracker_id"], how="left")
        ...
        results = prediction_idsw[["frame", "gt_id", "prediction", "gap", "tracker_id"]].merge(
            baseline_idsw[["frame", "gt_id", "baseline", "gap"]], left_on=["frame", "gt_id"], right_on=["frame", "gt_id"], how="outer")
        results.fillna(0,
                       inplace=True)
        results["gap"] = np.maximum(
            results["gap_x"].values, results["gap_y"].values)
        ...
        results_idtr["gap"] = np.maximum(
            results_idtr["gap_x"].values, results_idtr["gap_y"].values)
        results_idtr.frame -= 1
        TP_idtr = results_idtr[(
            (results_idtr.prediction == 0) & (results_idtr.baseline == 1))]
        FN_idtr = results_idtr[(
            (results_idtr.prediction == 1) & (results_idtr.baseline == 1))]
        FP_idtr = results_idtr[(
            (results_idtr.prediction == 1) & (results_idtr.baseline == 0))]

        self.analyse = Namespace(
            TP=TP, FN=FN, FP=FP, TP_idtr=TP_idtr, FP_idtr=FP_idtr, FN_idtr=FN_idtr)


def prepare_sequence(egomotion, mot, sequence):
        ...
        for frame in tqdm(np.unique(frames)):
            ...
            try:
                if y0 is not None:
                    ...
                    pos_t = pos*1.
                offset = np.array(H).dot(
                    np.array([[int(width/2), height, 1]]).T).T
    ...
    #             ax = sequence.plot_rgb(frame, show = False)
        ...
        if y0 is not None:
            new_pos = pix2real(H, pos*1., pixel_positions *
                               1., y0, img_width=width)
        else:
            new_pos = pos*1.
