...
class KalmanFilter():
    ...
    dt = 1/20., measurement_uncertainty = 2,
    ...
    verbose = False, frame = 1):
        ...
        if self.verbose: print("Initializing Kalman Filter:\ndf = {dt}".format(dt = dt))
    ...
    def smooth(self, zs, decrease_process_uncertainty = False, occluded = [] ): 
        ...
        # old_state = np.array([[self.kf.x[0],self.kf.x[1]]] )
        ...
        for i, z in enumerate(zs):
            ...
            self.kf.predict()
            predicted_trajectory.append(np.array([[self.kf.x[0],self.kf.x[1]]] ))
           
            if not np.isnan(z[0]):
              
                self.kf.update(z)
            smoothed_trajectory.append(np.array([[self.kf.x[0],self.kf.x[1]]] ))
    ...
    def step(self, z , decrease_process_uncertainty = False, frame = None):
        ...
        # return x, np.expand_dims(pred_x[:2], axis=0)
